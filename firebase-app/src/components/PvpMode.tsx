...
export function PvpMode({ service, onExit }: Props) {
  ...
  return <section id="page-pvp" className="isolate z-[60] pointer-events-auto flex flex-1 absolute inset-0 w-full h-full overflow-hidden bg-slate-950 text-white" style={{ backgroundImage: "linear-gradient(rgba(2,6,23,.68),rgba(2,6,23,.86)),url('https://i.postimg.cc/0QLbMBcH/2-3.png')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
    {view === 'select' && <div className="absolute inset-0 pointer-events-auto flex flex-col items-center justify-center z-40 bg-slate-950/55 px-4 overflow-y-auto"><h2 className="text-4xl md:text-6xl font-black mb-3 text-yellow-300">ศึกประลองความรู้ PVP</h2><p className="text-amber-200 font-bold mb-8">🔥 Real-time 1v1 บน Firestore 🔥</p><div className="flex flex-col md:flex-row gap-6 w-full max-w-3xl"><ModeCard icon="⚡" title="สุ่มท้าสู้ด่วน" text="จับคู่กับผู้เล่นที่กำลังรอในระบบ"><button type="button" onClick={() => void join(null)} className="pointer-events-auto w-full py-3.5 bg-gradient-to-b from-blue-500 to-indigo-600 font-black rounded-2xl">⚔️ เข้าประลองด่วน</button></ModeCard><ModeCard icon="🔒" title="ห้องส่วนตัว" text="สร้างหรือเข้าห้องด้วยรหัส 4 หลัก"><input value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="กรอกรหัสห้อง 4 หลัก" className="pointer-events-auto w-full px-4 py-3 text-center text-slate-900 text-2xl font-black rounded-xl mb-3" /><button type="button" onClick={() => void join(pin)} className="pointer-events-auto w-full py-3.5 bg-gradient-to-b from-red-500 to-rose-600 font-black rounded-2xl">⚔️ เข้าห้องท้าสู้ส่วนตัว</button></ModeCard></div>{message && <p className="text-red-300 font-bold mt-4">{message}</p>}<button type="button" onClick={onExit} className="pointer-events-auto mt-8 px-8 py-3 bg-slate-800 rounded-full font-bold">🚪 กลับสู่ Lobby</button></div>}
