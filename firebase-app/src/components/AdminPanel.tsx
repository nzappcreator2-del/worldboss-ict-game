...
export function AdminPanel({ service, onExit, confirmAction, downloadCsv = defaultDownload }: {
  ...
  return <section id="page-admin" style={{ display: 'block' }} className="min-h-screen w-full overflow-y-auto p-4 md:p-7">
    ...
    {status && <p role="status" className={`fixed inset-x-4 top-4 z-[200] mx-auto max-w-xl rounded-xl p-3 text-center font-bold shadow-2xl ${status.includes('แล้ว') ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-800'}`}>{status}</p>}
