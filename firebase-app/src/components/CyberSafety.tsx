...
export function CyberSafety({ service, onExit, onUserUpdate }: Props) {
  ...
  return (
    <section id="page-cyber-safety" className="flex-1 flex flex-col items-center justify-start relative overflow-y-auto py-8 px-4 w-full bg-slate-950" style={background ? { backgroundImage: `linear-gradient(rgba(15,23,42,.25),rgba(15,23,42,.45)),url('${background}')`, backgroundSize: 'cover', backgroundPosition: 'center' } : undefined}>
      ...
      {view === 'error' && <div className="glass-card bg-white p-8 max-w-lg text-center my-auto z-10"><p className="text-red-600 font-black mb-4">โหลดภารกิจไม่สำเร็จ</p><button type="button" onClick={() => void start()} className="bg-blue-600 text-white font-bold px-6 py-2 rounded-xl">ลองใหม่</button><button type="button" onClick={onExit} className="block mx-auto mt-4 underline">กลับ Lobby</button></div>}
      {['story', 'choices', 'feedback'].includes(view) && scenario && <div className="w-full max-w-4xl min-h-full flex flex-col justify-between z-10">
        ...
        <div className={`glass-card bg-slate-950/95 border-4 ${view === 'feedback' ? correct ? 'border-emerald-500' : 'border-rose-500' : 'border-cyan-400'} p-6 rounded-3xl text-white`}><div className="text-cyan-300 font-black mb-3">📖 เหตุการณ์: {scenario.title}</div><p className="whitespace-pre-wrap font-bold leading-relaxed">{view === 'feedback' ? feedback : scenario.text}</p><div className="flex justify-between items-center border-t border-white/10 pt-3 mt-4"><span className="font-bold">{service.getCurrentUser()?.avatar || '🧙‍♂️'} คู่หูผู้พิทักษ์</span>{view === 'story' && <button type="button" onClick={() => setView('choices')} className="bg-gradient-to-r from-cyan-400 to-blue-500 px-5 py-2.5 rounded-xl font-black">ร่วมตัดสินใจ</button>}{view === 'feedback' && <button type="button" onClick={continueAfterFeedback} className="bg-gradient-to-r from-cyan-400 to-blue-500 px-5 py-2.5 rounded-xl font-black">{correct ? index + 1 >= scenarios.length ? 'สรุปผล' : 'เดินทางต่อไป' : 'เลือกใหม่'}</button>}</div></div>
        <button type="button" onClick={onExit} className="mt-4 text-white/80 font-bold underline">กลับหน้าหลักเลือกโหมด (ไม่บันทึกแต้ม)</button>
      </div>}
      {view === 'victory' && <div className="w-full max-w-lg bg-gradient-to-b from-sky-500 to-blue-700 border-4 border-cyan-200 rounded-3xl p-6 text-center text-white z-10 my-auto"><div className="text-7xl mb-4">🛡️</div><h3 className="text-4xl font-black mb-2">ผู้พิทักษ์ภัยไซเบอร์!</h3><p className="text-cyan-100 font-bold mb-6">คุณช่วยน้องเซฟผ่านเหตุการณ์อันตรายบนโลกออนไลน์ครบแล้ว</p><div className="bg-sky-950/50 rounded-2xl p-5 mb-6"><p className="text-yellow-300 font-black">+{run.coins} Coins</p><p className="text-cyan-300 font-black">+{run.xp} EXP</p><p className="mt-2 font-bold">Cyber Shield: {run.shield}%</p></div>{saveError && <p className="text-red-200 font-bold mb-3">{saveError}</p>}<button type="button" disabled={saved} onClick={() => void save()} className="w-full py-4 bg-gradient-to-r from-yellow-400 to-yellow-500 disabled:opacity-60 text-yellow-950 font-black text-xl rounded-2xl">{saved ? '✅ บันทึกแล้ว' : '💾 ยืนยันบันทึกผลและรางวัล'}</button><button type="button" onClick={onExit} className="w-full py-3 mt-3 bg-black/20 font-bold rounded-xl">กลับสู่หน้าหลัก</button></div>}
    </section>
  )
}
